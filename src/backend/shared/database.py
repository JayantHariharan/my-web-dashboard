...
class AppRepository(BaseRepository):
    """Repository for app registry."""

    def __init__(self):
        super().__init__("app_registry")

    def get_active_apps(self) -> List[Dict[str, Any]]:
        """Get all active apps."""
        return self.find_many({"is_active": True}, order_by="name")

    def get_app_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        """Get app by name."""
        return self.find_one({"name": name})

    def get_app_by_route(self, route_path: str) -> Optional[Dict[str, Any]]:
        """Get app by route path."""
        return self.find_one({"route_path": route_path})
...
class UserActivityRepository(BaseRepository):
    """Repository for user app activity tracking."""

    def __init__(self):
        super().__init__("user_app_activity")
    ...
    def log_activity(
        ...
        data = {
            ...
            "created_at": (
                "CURRENT_TIMESTAMP" if self._is_postgres else "datetime('now')"
            ),
        }
        return self.create(data)
    ...
    def get_recent_activity(
        ...
        """Get recent app activity for user."""
        with self.get_cursor() as cursor:
            sql = (
                """
                SELECT uaa.*, ar.name as app_name
                FROM user_app_activity uaa
                JOIN app_registry ar ON uaa.app_id = ar.id
                WHERE uaa.user_id = %s
                ORDER BY uaa.last_accessed DESC, uaa.created_at DESC
                LIMIT %s
            """
                if self._is_postgres
                else """
...
class GameScoreRepository(BaseRepository):
    """Repository for game scores."""

    def __init__(self):
        super().__init__("game_scores")
    ...
    def get_leaderboard(self, game_name: str, limit: int = 10) -> List[Dict[str, Any]]:
...
game_score_repo = GameScoreRepository()
