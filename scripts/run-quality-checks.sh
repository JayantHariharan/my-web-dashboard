#!/bin/bash
# AI-Powered comprehensive quality & security checks
# Run this manually to verify code before pushing or in CI

set -e

echo "🚀 Running AI-powered comprehensive quality & security checks..."
echo ""
...
# 3. AI Security Analysis (separate, focused scan - uses OpenRouter)
check "AI Security Analysis"
if [ -n "$OPENROUTER_API_KEY" ]; then
    if python .github/scripts/claude_security_scan.py 2>/dev/null; then
        if [ -f "claude-security-report.json" ]; then
            ...
            if [ "$CRITICAL" -eq "0" ] && [ "$HIGH" -eq "0" ]; then
                pass "No Critical/High security issues (Total: $TOTAL)"
            else
                fail "$CRITICAL Critical and $HIGH High security issues found"
                echo "  See claude-security-report.json for details"
            fi
        else
            warn "No security report generated"
        fi
    else
        warn "Security analysis failed"
    fi
else
    warn "OPENROUTER_API_KEY not set (skipping AI security analysis)"
fi
echo ""

# 4. Check for hardcoded secrets (fast pattern match)
